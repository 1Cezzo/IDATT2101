...
public class EdmondsKarp {
    public static void main(String[] args) {
        Graph test = readGraphFromFile("exercise_7/flytgraf1.txt");
        int max = test.maxFlow(0, 7);
        System.out.println("Done! The maximum flow for the system is: " + max);
    }
    ...
    private static class Graph {
        ...
        public boolean increaseFlow(int src, int sink) {
            ...
            for (Node current = nodes[sink]; current.data.previous != null; current = current.data.previous) { //Goes backwards from sink to source to find the smallest rest capacity
            ...
            String pathToSink = "";
            for (Node current = nodes[sink]; current.data.previous != null; current = current.data.previous) { //Same loop, but now it increases flow and capacity for the edges
