...
public class opg9 {
  public static void main(String[] args) {
    ...
    Graph graph = createGraph("exercise_9/noder.txt", "exercise_9/kanter.txt");
    ...
    Graph reverse = createGraph("exercise_9/noder.txt", "exercise_9/kanter.txt");
    ...
    reverse = null;
    readInterestPointsFromFile("exercise_9/interessepkt.txt", graph);
  ...
  public static Node alt(Node from, Node to, int[][] toNode, int[][] fromNode) {
    ...
    from.data.distanceToTarget = heuristic(from, to, toNode, fromNode);
    queue.add(from);
    int nodesChecked = 0;
    while (!queue.isEmpty()) {
        ...
        for (Edge e = current.firstEdge; e != null; e = e.next) {
            if (!e.toNode.data.isChecked && e.toNode.data.distance > current.data.distance + e.weight) {
                ...
                d.distanceToTarget = heuristic(e.toNode, to, toNode, fromNode);
                d.priority = d.distance + d.distanceToTarget;

                if (d.heapIndex < 0) {
                    queue.add(e.toNode);
                } else {
                    queue.updatePosition(e.toNode);
                }
            }
        }
    }
  ...
  public static int heuristic(Node from, Node to, int[][] toNode, int[][] fromNode) {
      int distanceToTarget = Integer.MAX_VALUE;
      for (int i = 0; i < toNode[0].length; i++) {
          int estimate = toNode[to.number][i] - toNode[from.number][i];
          estimate = estimate < 0 ? 0 : estimate;
          int estimate2 = fromNode[from.number][i] - fromNode[to.number][i];
          estimate = estimate2 > estimate ? estimate2 : estimate;
          distanceToTarget = estimate > distanceToTarget ? estimate : distanceToTarget;
      }
